...
\subsection{산술 연산}
앞서 언급하였듯이, bash의 모든 변수는 기본적으로 문자열이기 때문에, 산술 연산을 하기 위해서는 별도의 구문을 사용하여야 한다. Bash에서 산술 연산을 할 수 있는 방법으로는 \code{expr} 명령어, \code{let} 명령어, 그리고 \code{$(( ))} 구문이 있다. 

\code{expr} 명령어는 아래와 같이 사용한다.
\begin{lstlisting}
    #!/bin/bash
    a=5
    b=3
...
\end{lstlisting}
중요한 점은 역따음표로 감싸줘야 한다는 거, 피연산자와 연산자 사이에 공백이 필요한 거, 그리고 곱셈 문자는 \code{\}로 이스케이프 처리해야 한다는 것이다. 또한 괄호 역시 이스케이프 처리를 해야 한다.

\code{let} 명령어는 아래와 같이 사용한다.
\begin{lstlisting}
    #!/bin/bash
    a=5
    b=3
